...
* **DBeaver 25.1.0** — это универсальный open-source-клиент для работы с базами данных, поддерживающий десятки СУБД (PostgreSQL, ClickHouse, DuckDB и др.) с удобным интерфейсом и SQL-редактором. Используется для работы с БД.
...
* **PlantUML** — инструмент для создания диаграмм (UML, ER, сетевых и др.) через простой текстовый синтаксис, автоматически генерирующий визуализации. Используется для описания и рендера диаграммы архитектуры проекта.
* **Cursor AI 1.0.0** — интеллектуальный редактор кода на базе ИИ (форк VS Code). Автоматизирует написание, рефакторинг и отладку, понимает контекст проекта, генерирует код по описанию на естественном языке и даже создает многофайловые приложения. Используется как IDE для всего проекта, от разработки Python и SQL-кода до подготовки документации.
